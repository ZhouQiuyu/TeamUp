/**
 * 
 */
package com.teamup.Constant;
/**
 *@author	created by Zozo E-mail:zozo@zhouqiuyu.cn
 *@date		2018年3月16日---下午10:14:33
 *@problem	
**/
public class ClientConstant {
	public static final String ACCOUNT_REGISTER_SUCCESS="100";//注册成功
	public static final String ACCOUNT_EXITED="101";//账号已存在
	public static final String ACCOUNT_REGISTER_FAIL="102";//注册失败
	public static final String ACCOUNT_LOGIN_SUCCCESS="200";//登录成功
	public static final String ACCOUNT_LOGIN_FAIL="201";//登录失败，账号密码不符
}
